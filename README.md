...
- **フォーラムタグ自動設定**: フォーラム作成時にタグの絵文字をリアクションとして追加し、ユーザーのリアクションでタグを自動適用
...
- `bot.enableForumTags`: フォーラムタグ機能を有効にするかどうか（デフォルト: true）
...
4. **フォーラムタグ自動設定**
   - フォーラム作成時、利用可能なタグに設定された絵文字を自動でリアクションとして追加
   - ユーザーがリアクションを押すと、対応するタグがフォーラム投稿に自動的に適用される
   - リアクションを外すと、対応するタグも自動的に削除される
   - 絵文字が設定されているタグのみが対象
   - フォーラム投稿内に利用可能なタグとリアクションの対応表を表示
